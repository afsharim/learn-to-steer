...
def apply_steering_vector(
    ...
    if x.shape[1] > 1:
        if individual_shift and vector.ndim > 1: # when the vector is composed of #samples vectors, and for each samples a different steering vector is to be picked
            SAMPLE_COUNTER += 1
            if vector.shape[0] > 1:
                vector = vector[min(SAMPLE_COUNTER-1, vector.shape[0]-1)]
    ...
    if individual_shift and vector.ndim > 1:
        if vector.shape[0] > 1:
            vector = vector[min(SAMPLE_COUNTER-1, vector.shape[0]-1)]
...
def apply_angular_steering(
    ...
    theta_t = torch.as_tensor(theta, device=x.device, dtype=x.dtype).reshape(())
...
def shift_hidden_states(
    ...
    **kwargs: Any,
    ...
    elif "reparo" in operation:
        def hook(module, input, output):
            if isinstance(output, tuple):
                outputs_ = apply_reparo_steering(
                    output[0],
...
def register_hooks(
    ...
    if "save_hidden_states" == hook_name:
    ...
    elif "shift_hidden_states" in hook_name or hook_name in {
        "add",
        "learned_steer",
        "angular_steer",
        "reparo",
        ...
        if "learned_steer" in hook_name: # <--- ADDED THIS
            ...
            # import pdb; pdb.set_trace()
            # Example for kernel encoder
            spca_llava_path ="/research/hal-afsharim/learn-to-steer/Hallucination/POPE/hallucination/spca_models/spca_model_llava_14_all_pope_train_-1.pth"
            spca_qwen_path = "/research/hal-afsharim/learn-to-steer/Hallucination/POPE/hallucination/spca_models/spca_model_qwen_17_all_pope_train_-1.pth"
            spca = torch.load(spca_llava_path if 'llava' in model.model_name_or_path.lower() else spca_qwen_path, map_location=device)
            x_train_normalized = spca["x_train_normalized"].to(device)
            n_train = x_train_normalized.shape[0]
            mean_h = spca["mean_x"].to(device)
            if mean_h.dim() == 1:
                mean_h = mean_h.unsqueeze(0)
            
            H_train = torch.eye(n_train, device=device) - (1/n_train) * torch.ones((n_train, n_train), device=device)
            K_train = spca["K"].to(device)
            alpha_normalized = spca["alpha_normalized"].to(device)
            gamma = 1
            
            def encode_kernel(h, x_train_normalized, K_train, H_train,alpha_normalized, gamma):
                dist_sq = torch.cdist(h, x_train_normalized,p=2)**2
                K_input = torch.exp(-gamma*dist_sq)
                n_input = h.shape[0]
                n_train = x_train_normalized.shape[0]
                ones_m_n = torch.ones((n_input, n_train), device=h.device)/n_train
                
                K_input_c = (K_input - ones_m_n @ K_train )@ H_train
                z = K_input_c @ alpha_normalized
                return z
...
def hooks_postprocessing(
    ...
    if "save_hidden_states" in hook_name:
        ...
        # Steering hooks do not require postprocessing callbacks.
        hook_postprocessing_function = None
