...
def set_angular_steering(compass_vector: torch.Tensor = None, theta: float = None, global_mean: torch.Tensor = None):
    """
    Call this once before generation to set your parameters.
    Pass your loaded global_center.pth into global_mean.
    """
    global ANGULAR_COMPASS, PREDICTED_THETA, GLOBAL_MEAN
...
def apply_angular_steering(
    ...
    global SAMPLE_COUNTER, GLOBAL_MEAN
    ...
    # theta_ratio is treated as an absolute rotation in radians
    theta_ratio = float(theta)
    
    # FIX: Prepare the theta tensor for torch trig functions
    theta_t_tensor = torch.tensor(theta_ratio, device=x.device, dtype=x.dtype)
    
    # FIX: Load global mean safely if it exists
    mu = GLOBAL_MEAN.to(x.device).to(x.dtype) if GLOBAL_MEAN is not None else 0
    ...
    if x.shape[1] > 1:
        ...
        if start_prompt_token_idx > 0 or start_prompt_token_idx == -1:
            ...
            # FIX: Center the data (Subtract global mean)
            x_c = x_ - mu
            
            h_norm = torch.norm(x_c, dim=-1, keepdim=True)
            e1 = x_c / (h_norm + 1e-8)
            
            # FIX: Gram-Schmidt MUST use z_normed, not raw z
            projection = torch.sum(z_normed * e1, dim=-1, keepdim=True) * e1
            z_perp = z_normed - projection
            e2 = z_perp / (torch.norm(z_perp, dim=-1, keepdim=True) + 1e-8)
            
            # Apply Rotation
            cos_theta = torch.cos(theta_t_tensor)
            sin_theta = torch.sin(theta_t_tensor)
            
            x_rotated_c = h_norm * (cos_theta * e1 + sin_theta * e2)
            
            # FIX: Un-center (Add global mean back)
            x_final = x_rotated_c + mu
            
            # FIX: Preserve exact original norm
            orig_norm = torch.norm(x_, dim=-1, keepdim=True)
            final_norm = torch.norm(x_final, dim=-1, keepdim=True)
            x_final = x_final * (orig_norm / (final_norm + 1e-8))
            
            x[:, start_prompt_token_idx:, :] = x_final
            return x
    ...
    # 1. Calculate the natural dynamic angle (Using x_c and z_normed)
    ...
    # 2. Gram-Schmidt (Using z_normed)
    projection = torch.sum(z_normed * e1, dim=-1, keepdim=True) * e1
    z_perp = z_normed - projection
    e2 = z_perp / (torch.norm(z_perp, dim=-1, keepdim=True) + 1e-8)
    
    # 3. Apply Rotation (Explicit theta)
    cos_theta = torch.cos(theta_t_tensor)
    sin_theta = torch.sin(theta_t_tensor)
    
    x_rotated_c = h_norm * (cos_theta * e1 + sin_theta * e2)
    
    # 4. Un-center and restore exact norm
    x_final = x_rotated_c + mu
    
    orig_norm = torch.norm(x, dim=-1, keepdim=True)
    final_norm = torch.norm(x_final, dim=-1, keepdim=True)
    
    x = x_final * (orig_norm / (final_norm + 1e-8))
    return x
# ==========================================
########################################################
...
def register_hooks(
    ...
    if "save_hidden_states" == hook_name:
        ...
        if "learned_steer" in hook_name: # <--- ADDED THIS
            ...
            if operation == "angular_steer":
                global_center_tensor = None
                if hasattr(args, "global_center_path") and args.global_center_path is not None:
                    global_center_tensor = torch.load(args.global_center_path)["steering_vector"]
            
                set_angular_steering(
                    compass_vector=vector,
                    theta=args.steering_alpha,
                    global_mean=global_center_tensor
                )
